...
public partial class SecurityAnalyzerView : UserControl
{
    ...
    {
        InitializeComponent();
        DragDropHelper.EnableFileDrop(InputPathTextBox, _ => UpdateAnalyzeButton(), acceptDirectories: true);
    }
        ...
        try
        {
            ...
            else
            {
                ...
                ShowStatus(result.Features.Count > 0
                    ? string.Format(loc["Security_SingleResultDetected"], result.FileName, result.Features.Count)
                    : string.Format(loc["Security_SingleNoFeatures"], result.FileName), isError: false);
