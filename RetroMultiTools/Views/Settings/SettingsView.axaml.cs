...
namespace RetroMultiTools.Views.Settings;
...
public partial class SettingsView : UserControl
{
    ...
    private void LoadBigPictureSettings()
    {
        ...
        BigPictureStyleCombo.Items.Clear();
        BigPictureStyleCombo.Items.Add(new ComboBoxItem { Content = loc["Settings_BigPictureStyleClassic"], Tag = "Classic" });
        BigPictureStyleCombo.Items.Add(new ComboBoxItem { Content = loc["Settings_BigPictureStyleModern"], Tag = "Modern" });
        BigPictureStyleCombo.SelectedIndex = AppSettings.Instance.BigPictureStyle == "Modern" ? 1 : 0;
    ...
    private void BigPictureStyleCombo_SelectionChanged(object? sender, SelectionChangedEventArgs e)
    {
        if (_isInitializing) return;
        if (BigPictureStyleCombo.SelectedItem is ComboBoxItem item && item.Tag is string style)
            AppSettings.Instance.BigPictureStyle = style;
    }
    ...
    private void LoadUpdateSettings()
    {
        ...
        // In sandboxed environments (Flatpak, Snap), the package manager handles
        // updates so the in-app update mechanism should be disabled.
        if (AppUpdater.IsRunningInSandbox())
        {
            CheckForUpdatesOnStartupCheck.IsEnabled = false;
            CheckForUpdatesButton.IsEnabled = false;
            UpdateStatusText.Text = Localization.LocalizationManager.Instance["Settings_UpdateManagedByPackageManager"];
        }
    }
    ...
    private async void CheckForUpdatesButton_Click(object? sender, Avalonia.Interactivity.RoutedEventArgs e)
    {
        ...
        {
            System.Diagnostics.Trace.WriteLine($"[SettingsView] Update check unexpected error: {ex.Message}");
            _pendingUpdate = null;
            UpdateStatusText.Text = Localization.LocalizationManager.Instance["Settings_UpdateNetworkError"];
        }
        finally
    ...
    private void RestartTourButton_Click(object? sender, Avalonia.Interactivity.RoutedEventArgs e)
    {
