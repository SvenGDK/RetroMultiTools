...
public partial class PatchCreatorView : UserControl
{
    ...
    {
        InitializeComponent();
        OutputFileTextBox.TextChanged += (_, _) => UpdateCreateButton();
        DragDropHelper.EnableFileDrop(OriginalFileTextBox, _ => UpdateCreateButton());
        DragDropHelper.EnableFileDrop(ModifiedFileTextBox, _ => UpdateCreateButton());
    }
    ...
    {
        CreateButton.IsEnabled = _analysis != null && !_analysis.IsIdentical &&
                                 _analysis.CanCreateIps && !string.IsNullOrEmpty(OutputFileTextBox.Text);
    }
