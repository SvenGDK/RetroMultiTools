...
namespace RetroMultiTools.Views.Conversion;
...
public partial class RomFormatConverterView : UserControl
{
    ...
    {
        InitializeComponent();
        OutputPathTextBox.TextChanged += (_, _) => UpdateConvertButton();
        DragDropHelper.EnableFileDrop(InputPathTextBox, _ => UpdateConvertButton(), acceptDirectories: true);
    }

    /// <summary>
    /// Pre-fills the input path with the specified file and populates the conversion options.
    /// </summary>
    public void SetInputFile(string filePath)
    {
        if (string.IsNullOrEmpty(filePath) || !File.Exists(filePath)) return;
        // Ensure single-file mode is selected
        if (SingleModeRadio.IsChecked != true)
            SingleModeRadio.IsChecked = true;
        InputPathTextBox.Text = filePath;
        PopulateSingleConversions(filePath);
        UpdateOutputPath();
        UpdateConvertButton();
    }
