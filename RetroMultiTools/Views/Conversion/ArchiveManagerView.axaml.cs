...
public partial class ArchiveManagerView : UserControl
{
    ...
    private void OperationRadio_Checked(object? sender, Avalonia.Interactivity.RoutedEventArgs e)
    {
        ...
        RefreshLabels(overrideExtract: sender == ExtractRadio);
    }
    ...
    private void ModeRadio_Checked(object? sender, Avalonia.Interactivity.RoutedEventArgs e)
    {
        if (InputLabel == null) return;
        if (sender is not RadioButton rb || rb.IsChecked != true) return;
        RefreshLabels(overrideBatch: sender == BatchModeRadio);
    }

    private void RefreshLabels(bool? overrideExtract = null, bool? overrideBatch = null)
    {
        var loc = LocalizationManager.Instance;
        bool extract = overrideExtract ?? IsExtractMode;
        bool batch = overrideBatch ?? IsBatchMode;
