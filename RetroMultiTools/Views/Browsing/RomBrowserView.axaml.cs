...
public partial class RomBrowserView : UserControl
{
    ...
    private void UpdateContextMenuForSelection()
    {
        ...
        bool isSingleSelection = RomDataGrid.SelectedItems.Count == 1;
        ...
        // Single-selection-only items
        ContextRenameItem.IsEnabled = isSingleSelection;
        ContextInspectItem.IsEnabled = isSingleSelection;
        ContextHexViewItem.IsEnabled = isSingleSelection;
        ContextChecksumItem.IsEnabled = isSingleSelection;
        ContextGoodToolsItem.IsEnabled = isSingleSelection;
        ContextPatchItem.IsEnabled = isSingleSelection;
        ContextOpenFolderItem.IsEnabled = isSingleSelection;
    }
    ...
    private void ContextConvert_Click(object? sender, Avalonia.Interactivity.RoutedEventArgs e)
    {
        if (RomDataGrid.SelectedItem is not RomInfo selectedRom) return;
        if (TopLevel.GetTopLevel(this) is MainWindow mainWindow)
            mainWindow.NavigateToViewWithFile("formatconv", selectedRom.FilePath);
    }
    ...
    private void ContextTrim_Click(object? sender, Avalonia.Interactivity.RoutedEventArgs e)
    {
        if (RomDataGrid.SelectedItem is not RomInfo selectedRom) return;
        if (TopLevel.GetTopLevel(this) is MainWindow mainWindow)
            mainWindow.NavigateToViewWithFile("trimmer", selectedRom.FilePath);
    }
    ...
    {
        if (RomDataGrid.SelectedItem is not RomInfo) return;
        if (TopLevel.GetTopLevel(this) is MainWindow mainWindow)
            mainWindow.NavigateToView("dumpverifier");
    }

    private void ContextInspect_Click(object? sender, Avalonia.Interactivity.RoutedEventArgs e)
    {
        if (RomDataGrid.SelectedItem is not RomInfo selectedRom) return;
        if (TopLevel.GetTopLevel(this) is MainWindow mainWindow)
            mainWindow.NavigateToViewWithFile("inspector", selectedRom.FilePath);
    }

    private void ContextHexView_Click(object? sender, Avalonia.Interactivity.RoutedEventArgs e)
    {
        if (RomDataGrid.SelectedItem is not RomInfo selectedRom) return;
        if (TopLevel.GetTopLevel(this) is MainWindow mainWindow)
            mainWindow.NavigateToViewWithFile("hexviewer", selectedRom.FilePath);
    }

    private void ContextChecksum_Click(object? sender, Avalonia.Interactivity.RoutedEventArgs e)
    {
        if (RomDataGrid.SelectedItem is not RomInfo selectedRom) return;
        if (TopLevel.GetTopLevel(this) is MainWindow mainWindow)
            mainWindow.NavigateToViewWithFile("checksum", selectedRom.FilePath);
    }

    private void ContextGoodTools_Click(object? sender, Avalonia.Interactivity.RoutedEventArgs e)
    {
        if (RomDataGrid.SelectedItem is not RomInfo selectedRom) return;
        if (TopLevel.GetTopLevel(this) is MainWindow mainWindow)
            mainWindow.NavigateToViewWithFile("goodtools", selectedRom.FilePath);
    }

    private void ContextPatch_Click(object? sender, Avalonia.Interactivity.RoutedEventArgs e)
    {
        if (RomDataGrid.SelectedItem is not RomInfo selectedRom) return;
        if (TopLevel.GetTopLevel(this) is MainWindow mainWindow)
            mainWindow.NavigateToViewWithFile("patcher", selectedRom.FilePath);
    }

    private void ContextOpenFolder_Click(object? sender, Avalonia.Interactivity.RoutedEventArgs e)
    {
        try
        {
            if (RomDataGrid.SelectedItem is not RomInfo selectedRom) return;
            string? folder = Path.GetDirectoryName(selectedRom.FilePath);
        ...
        {
            StatusText.Text = string.Format(LocalizationManager.Instance["Browser_OpenFolderError"], ex.Message);
        }
    }

    private async void ContextRename_Click(object? sender, Avalonia.Interactivity.RoutedEventArgs e)
    {
        try
        {
            if (RomDataGrid.SelectedItem is not RomInfo selectedRom) return;
            ...
            var dialog = new RetroMultiTools.Views.Dialogs.InputDialog(
                LocalizationManager.Instance["Browser_RenamePromptTitle"],
                Path.GetFileName(selectedRom.FilePath));
            var newName = await dialog.ShowDialog(parentWindow);

            if (string.IsNullOrWhiteSpace(newName)) return;

            string? directory = Path.GetDirectoryName(selectedRom.FilePath);
            ...
            {
                StatusText.Text = string.Format(LocalizationManager.Instance["Browser_RenameError"],
                    LocalizationManager.Instance["Renamer_ErrorFileExists"]);
                return;
            }

            File.Move(selectedRom.FilePath, newPath);

            StatusText.Text = string.Format(LocalizationManager.Instance["Browser_RenameSuccess"], newName);
            await ScanCurrentFolder();
        }
        catch (Exception ex) when (ex is IOException or UnauthorizedAccessException)
        {
            StatusText.Text = string.Format(LocalizationManager.Instance["Browser_RenameError"], ex.Message);
        }
        catch (Exception ex)
        {
            System.Diagnostics.Trace.WriteLine($"[RomBrowser] ContextRename_Click failed: {ex.Message}");
        }
    }
