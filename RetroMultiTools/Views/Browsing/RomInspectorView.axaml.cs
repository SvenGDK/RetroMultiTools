...
public partial class RomInspectorView : UserControl
{
    ...
    /// <summary>
    /// Opens and inspects the specified ROM file as if the user selected it via the file picker.
    /// </summary>
    public void SetInputFile(string filePath)
    {
        if (string.IsNullOrEmpty(filePath) || !File.Exists(filePath)) return;
        // Simulate button click by directly loading the file
    ...
    private async void OpenRomButton_Click(object? sender, Avalonia.Interactivity.RoutedEventArgs e)
    {
        ...
        LoadRomFile(files[0].Path.LocalPath);
    }

    private async void LoadRomFile(string path)
    {
        ...
        {
            System.Diagnostics.Trace.WriteLine($"[RomInspector] LoadRomFile failed: {ex.Message}");
            ProgressText.Text = string.Format(LocalizationManager.Instance["Common_ErrorFormat"], ex.Message);
        }
        finally
        {
