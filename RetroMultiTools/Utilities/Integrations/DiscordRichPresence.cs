...
namespace RetroMultiTools.Utilities.Integrations;
...
public static class DiscordRichPresence
{
    ...
    {
        // In Flatpak, Discord IPC sockets are accessed via the host XDG_RUNTIME_DIR
        // mapped through the sandbox. Try standard named pipes first, then fall back
        // to Unix domain sockets at the XDG_RUNTIME_DIR path if available.
        for (int i = 0; i < 10; i++)
        {
            try
            {
                ...
                // On Linux inside Flatpak, try the socket path from XDG_RUNTIME_DIR first
                if (OperatingSystem.IsLinux())
                {
                    string? xdgDir = Environment.GetEnvironmentVariable("XDG_RUNTIME_DIR");
                    if (!string.IsNullOrEmpty(xdgDir))
                    {
                        // Discord places its IPC socket at {XDG_RUNTIME_DIR}/discord-ipc-{i}
                        // or inside a sub-directory like app/com.discordapp.Discord/
                        string socketPath = Path.Combine(xdgDir, pipeName);
                        if (!System.IO.File.Exists(socketPath))
                        {
                            string appSocketPath = Path.Combine(xdgDir, "app", "com.discordapp.Discord", pipeName);
                            if (System.IO.File.Exists(appSocketPath))
                        ...
                        if (System.IO.File.Exists(socketPath))
                        {
                            ...
                            try
                            {
                                ...
                                _pipeStream = new NetworkStream(socket, ownsSocket: true);
                                // NetworkStream now owns the socket — clear local ref
                                // so the catch block does not double-dispose it.
                                socket = null;
                            ...
                            {
                                // Dispose the socket if it was never handed to NetworkStream
                                socket?.Dispose();
                                // Fall through to named pipe approach
                            }
                        }
                    }
                }
