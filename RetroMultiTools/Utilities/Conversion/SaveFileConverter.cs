namespace RetroMultiTools.Utilities.Conversion;
...
public static class SaveFileConverter
{
    ...
    private static byte[] PadToPowerOfTwo(byte[] data)
    {
        ...
        long target = 1;
        while (target < data.Length && target > 0) target <<= 1;
        if (target < 0) target = MaxFileSize; // cap on overflow
    ...
    private static byte[] TrimTrailing(byte[] data, byte padByte)
    {
        ...
        long aligned = 1;
        while (aligned < end && aligned > 0) aligned <<= 1;
        if (aligned < 0) aligned = MaxFileSize; // cap on overflow
